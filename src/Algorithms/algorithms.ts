...
import { steps, markNode, useEdge, GeneralCommand } from "./commands.js";
 ...
 * @param node 
 */
function getNodesAndEdges(node : Node, onlyWeighted : boolean){
	let edges : Edge[] = []
	...
	for(let i = 0; i < possibleNodes.length; i++){
		...
		currentNode.edges.forEach(edge => {
			if((!onlyWeighted || edge.hasWeight()) && !edges.includes(edge)){
				...
				if(nbour !== null)possibleNodes.push(nbour)
			}
		})
	}
...
export function dfs(start : Node){
	let nodeStack : Node[] = [start]
	let edgeStack : Edge[] = []
	while(nodeStack.length > 0){
		const currentNode = nodeStack[nodeStack.length-1]
		if(!currentNode.wasVisited()){	
			if(edgeStack.length > 0)steps.addStep(useEdge(edgeStack.pop()!))
			steps.addStep(markNode(currentNode))
		}

		//typescript types find() as not also returning undefined? 
		const nextNode : Node | undefined = currentNode.getNeighbours().find(nbour => !nbour.wasVisited())
		steps.addStep(createExplanation(
			false,
			"Check for a next unvisited neighbour of ", 
			nodeRepresentation(currentNode), 
			": ", 
			nextNode !== undefined?nodeRepresentation(nextNode):"there are none"
		))

		if(nextNode){
			nodeStack.push(nextNode)
			const nbour = currentNode.getEdgeConnectingTo(nextNode)
			if(nbour !== undefined) edgeStack.push(nbour)
		}
		else {
			nodeStack.pop()
			edgeStack.pop()
 ...
 * @param edges 
 */
export function kruskal(node : Node){
	...
	function hasCyclesDFS(node : Node){
		usedNodes.push(node)
		for(let edge of node.edges){
			if((edge.wasVisited() || edge == smallestEdge) && !usedEdges.includes(edge)){
				usedEdges.push(edge)
				const neigh = edge.getNeighbourOf(node)!
				if(usedNodes.includes(neigh))return true
				if(hasCyclesDFS(neigh))return true
			}
		}
		return false
	}
...
export function prim(node : Node){
	function isPossible(edge : Edge){
		return !edge.wasVisited() && edge.hasWeight() && edge.nodes.some(n => !n.wasVisited())
	}
	function findPossibleEdges(node : Node){
	...
	if(possibleEdges.length !== 0)steps.addStep(markNode(node))

	while(possibleEdges.length > 0){
		const currentEdge = possibleEdges.pop()!
		if(!isPossible(currentEdge))continue
