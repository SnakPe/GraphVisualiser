...
export class Edge{
	...
	constructor(node1 : Node, node2 : Node){
		...
		if(node1 != node2)this.createNormalEdgeSVG()
		else this.createReturningEdgeSVG()
	...
	private createEdgeSVG(svgtype : "path"|"line"){
	...
	refreshSVG(){
		...
		if(this.nodes[0] === this.nodes[1])refreshReturningEdge()
		else refreshNormalEdge()
		if(this.weight !== null)refreshWeightedEdge()
	...
	calcNormalLineEnds(){
		...
		if(xDiff == 0)return [this.nodes[0].x,this.nodes[0].y,this.nodes[1].x,this.nodes[1].y]
	...
	*/
	getNeighbourOf(node : Node){
		switch (node) {
			case this.nodes[0]:
				return this.nodes[1]
			case this.nodes[1]:
				if(!this.directed)return this.nodes[0]
		}
		return null;
